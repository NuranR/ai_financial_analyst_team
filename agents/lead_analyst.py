    ...
    def analyze(self, ticker: str, **kwargs) -> AgentResult:
        ...
        try:
            ...
            if enable_news:
                ...
                news_result = self.data_journalist.analyze(
                    ticker, days_back=14, max_articles=20
                )
            ...
            if enable_quant:
                ...
                quant_result = self.quant_analyst.analyze(
            ...
            synthesis_prompt = LEAD_ANALYST_SYNTHESIS_PROMPT.format(
                ...
                regulatory_analysis=self._format_agent_result(
                    agent_results.get("regulatory")
                ),
                current_price=current_price,
                market_cap=market_cap,
            )

            comprehensive_analysis = self._call_llm(
                synthesis_prompt, LEAD_ANALYST_SYSTEM_PROMPT
            )
            ...
            metadata = {
                ...
                "individual_confidences": {
                    k: v.confidence_score for k, v in agent_results.items() if v
                },
                "current_price": current_price,
                "market_cap": market_cap,
                "analysis_period": analysis_period,
                "analysis_timestamp": datetime.now().isoformat(),
                "recommendation_basis": self._extract_recommendation_basis(
                    agent_results
                ),
            }
...
"""
    ...
    def _calculate_overall_confidence(
        ...
        completeness_bonus = (
            len([r for r in agent_results.values() if r and not r.errors]) * 0.05
        )

        final_score = (weighted_score / total_weight) + completeness_bonus
        return min(final_score, 1.0)

    def _extract_recommendation_basis(
        self, agent_results: Dict[str, AgentResult]
    ) -> Dict[str, str]:
        ...
        if news_result and not news_result.errors:
            basis["news_sentiment"] = (
                "Positive" if "positive" in news_result.analysis.lower() else "Mixed"
            )
        ...
        if reg_result and not reg_result.errors:
            basis["regulatory_health"] = reg_result.metadata.get(
                "regulatory_health", "Unknown"
            )
