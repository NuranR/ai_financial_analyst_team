...
class RegulatorSpecialistAgent(BaseAgent):
    ...
    def analyze(self, ticker: str, **kwargs) -> AgentResult:
        ...
        try:
            ...
            filing_data = self.sec_fetcher.get_company_filings(
                ticker=ticker, filing_types=filing_types, limit=max_filings
            )
            ...
            analysis_data = self._format_filing_data_for_analysis(
                filing_data, filing_insights
            )
            ...
            analysis_prompt = REGULATOR_SPECIALIST_ANALYSIS_PROMPT.format(
                ...
                ticker=ticker,
                filing_type=filing_insights["filing_summary"].get(
                    "latest_filing", "Various"
                ),
                filing_date=filing_insights["filing_summary"].get(
                    "latest_filing_date", "Unknown"
                ),
            ...
            if unusual_events:
                ...
                for event in unusual_events:
                    analysis_prompt += (
                        f"• {event['filing_date']}: {event['description']}\n"
                    )

            analysis = self._call_llm(
                analysis_prompt, REGULATOR_SPECIALIST_SYSTEM_PROMPT
            )
            ...
            metadata = {
                ...
                "risk_signals": filing_insights["compliance_assessment"][
                    "risk_signals"
                ],
    ...
    def _format_filing_data_for_analysis(
        self, filing_data: Dict[str, Any], filing_insights: Dict[str, Any]
    ) -> Dict[str, str]:
...
"""
    ...
    def _assess_compliance_score(self, filing_insights: Dict[str, Any]) -> float:
        ...
        health_scores = {"good": 0.3, "fair": 0.1, "concerning": -0.2, "unknown": 0.0}
